...
/* other options */
  ...
  { "--pid",        eslARG_REAL,       "0.20", NULL, NULL,      NULL,  NULL,  NULL,              "pid threshold for accepting a sequence",                      12 },
  { "--qsize",      eslARG_INT,        "5", NULL, NULL,         NULL,  NULL,  NULL,              "num queries per thread",                                      12 },
  { "--filter",     eslARG_NONE,       FALSE, NULL, NULL,       NULL,  NULL,  NULL,              "filtering results or not",                                    12 },
#ifdef HMMER_THREADS
...
#ifdef HMMER_THREADS
#define BLOCK_SIZE 1104224
#define QBLOCK_SIZE 7200
#endif
...
assign_master(ESL_GETOPTS *go, struct cfg_s *cfg, int *num_results)
{
  ...
  *num_results = qdb->count;
  ...
  /* Open the sequence database file  */
...
thread_loop(ESL_THREADS *threadObj, int *num_cores)
{
  ...
  for(int i=0; i<*num_cores; i++)
  {
    ...
    {
      if (esl_opt_GetBoolean(info->go, "--filter"))
      {
      ...
      {
        all_decisions[result_ctr] = (info->result[j]).decision;
        result_ctr++;
      }
    }
  }
...
{  
  ...
  for(q_ctr=0; q_ctr<info->num_queries; q_ctr++)
  {
    ...
    p7_SingleBuilder(bld, qsq, bg, NULL, NULL, NULL, &om); /* bypass HMM - only need model */ // info->bld
    ...
    // fprintf(stderr, "Worker %d on Query %s of length %d took %f s after %d seqs\n", workeridx, qsq->name, qsq->n, w->elapsed, i);
            ...
            for (d = 0; d < nc; d++)
            {
                  ...
                  */
                  ...
                  if ((status = rescore_domain_cust(ddef, om, sq, qsq, fwd, bck, i2, j2, TRUE, bg, long_target, bg_tmp, scores_arr, fwd_emissions_arr, threshold, result, nullsc, incE, db_size)) == eslOK)
                    last_j2 = j2;
                  if ((result->decision == REJECT) && (!filter)) break; // breaking out of first loop over domains

            }

            p7_spensemble_Reuse(ddef->sp);
            p7_trace_Reuse(ddef->tr);
            if ((result->decision == REJECT) && (!filter)) break; // must break out of second loop!!
        }
        else
        {
            ...
            if ((result->decision == REJECT) && (!filter)) break;
        }
        i = -1;
        triggered = FALSE;
    }
      ...
      RESULT_INFO *result, float *nullsc, float *incE, float *db_size)
{
  ...
  if (decision == REJECT)
  {
    ...
    dom->lnP      = esl_exp_logsurv (dom->bitscore,  om->evparam[p7_FTAU], om->evparam[p7_FLAMBDA]);
    eval = dom->lnP + log(*db_size);
...
find_pid(P7_TRACE *tr, const ESL_SQ *sq, const ESL_SQ *qsq, float *threshold, float *pid, char *decision)
{
  ...
  /* Walk through the trace, check for match state and canonical residue */
  for(j=0; j<tr->N; j++)
  {
